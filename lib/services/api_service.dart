...
class ApiService {
  ...
  Future<Appointment> getAppointmentById({
    ...
    try {
      ...
      final response = await getWithAuth('/appointments/$appointmentId', token);
      ...
      if (response.statusCode == 200) {
        try {
          ...
          print('❌ Error parsing appointment response: $e');
          print('Response body: ${response.body}');
          throw Exception('خطأ في معالجة استجابة الخادم');
        }
      } else if (response.statusCode == 404) {
        ...
        try {
          final error = jsonDecode(response.body);
          final message = error['message'] ?? 'فشل تحميل الموعد';
          throw Exception(message);
        } catch (e) {
          throw Exception('فشل تحميل الموعد (${response.statusCode})');
        }
      }
    } catch (e) {
...
extension VideoSessionApi on ApiService {
  ...
  Future<PaymentIntentResponse> createPaymentIntent({
    ...
    try {
      ...
      final response = await postWithAuth('/payments/intent', {
        'appointmentId': appointmentId,
      }, token);
      ...
      if (response.statusCode != 201 && response.statusCode != 200) {
        try {
          if (response.body.isNotEmpty) {
            final error = jsonDecode(response.body);
            throw Exception(error['message'] ?? 'فشل إنشاء نية الدفع');
          } else {
            throw Exception('فشل إنشاء نية الدفع (${response.statusCode})');
          }
        } catch (e) {
          if (e is Exception) rethrow;
          throw Exception('فشل إنشاء نية الدفع (${response.statusCode})');
      ...
      if (response.body.isEmpty || response.body.trim().isEmpty) {
        print('❌ Empty response body from payment intent endpoint');
        throw Exception('استجابة فارغة من الخادم. يرجى المحاولة مرة أخرى.');
      }
  ...
  Future<Payment?> getPaymentStatus({
    ...
    try {
      ...
      final response = await getWithAuth('/payments/$appointmentId', token);

      if (response.statusCode == 404) {
        return null; // لا يوجد دفع مرتبط بالموعد
      }
      ...
      if (response.statusCode != 200) {
        try {
          if (response.body.isNotEmpty) {
            final error = jsonDecode(response.body);
            throw Exception(error['message'] ?? 'فشل جلب حالة الدفع');
          } else {
            throw Exception('فشل جلب حالة الدفع (${response.statusCode})');
          }
        } catch (e) {
          if (e is Exception) rethrow;
          throw Exception('فشل جلب حالة الدفع (${response.statusCode})');
      ...
      // التحقق من أن الاستجابة ليست فارغة
      if (response.body.isEmpty || response.body.trim().isEmpty) {
        print('❌ Empty response body from payment status endpoint');
        return null; // لا يوجد دفع مرتبط بالموعد
      }
      ...
      if (e is Exception) rethrow;
      throw Exception('خطأ في جلب حالة الدفع: ${e.toString()}');
    }
  }

  /// جلب سجل مدفوعات المستخدم
  Future<List<Payment>> getMyPaymentHistory(String token) async {
    try {
      if (token.isEmpty) {
        throw Exception('غير مصرح - يرجى تسجيل الدخول');
      }

      final response = await getWithAuth('/payments/my-history', token);

      if (response.statusCode != 200) {
        try {
          if (response.body.isNotEmpty) {
            final error = jsonDecode(response.body);
            throw Exception(error['message'] ?? 'فشل جلب سجل المدفوعات');
          } else {
            throw Exception('فشل جلب سجل المدفوعات (${response.statusCode})');
          }
        } catch (e) {
          if (e is Exception) rethrow;
          throw Exception('فشل جلب سجل المدفوعات (${response.statusCode})');
      ...
      if (e is Exception) rethrow;
      throw Exception('خطأ في جلب سجل المدفوعات: ${e.toString()}');
    }
  }

  // --- Support Tickets ---

  Future<List<SupportTicket>> getMyTickets(String token) async {
    try {
      final response = await getWithAuth('/support-tickets/my-tickets', token);
      if (response.statusCode == 200) {
  ...
  Future<SupportTicket> createTicket(
    ...
    try {
      final response = await postWithAuth('/support-tickets', {
        'subject': subject,
        'category': category,
  ...
  Future<Map<String, dynamic>> getTicketDetails(
    ...
    try {
      final response = await getWithAuth('/support-tickets/$ticketId', token);
      if (response.statusCode == 200) {
      ...
      throw Exception('فشل في جلب تفاصيل التذكرة');
    } catch (e) {
      rethrow;
    }
  }
  ...
  Future<TicketReply> addTicketReply(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        return TicketReply.fromJson(json.decode(response.body));
      }
  ...
  Future<void> forgotPassword(String email) async {
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          final error = jsonDecode(response.body);
          final message = error['message'] ?? 'البريد الإلكتروني غير مسجل';
          throw Exception(message);
        } catch (e) {
        ...
        try {
          final error = jsonDecode(response.body);
          final message = error['message'] ?? 'فشل في إرسال رمز التحقق';
          throw Exception(message);
        } catch (e) {
          throw Exception('فشل في إرسال رمز التحقق (${response.statusCode})');
        }
      }
    } catch (e) {
      if (e is Exception) rethrow;
  ...
  Future<void> verifyOtpAndResetPassword(
    ...
    try {
      final response = await post('/auth/verify-otp-reset-password', {
        'email': email.trim().toLowerCase(),
        'otpCode': otpCode,
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          final error = jsonDecode(response.body);
          final message =
              error['message'] ?? 'رمز التحقق غير صحيح أو منتهي الصلاحية';
          throw Exception(message);
        } catch (e) {
        ...
        try {
          final error = jsonDecode(response.body);
          final message = error['message'] ?? 'فشل في إعادة تعيين كلمة السر';
          throw Exception(message);
        } catch (e) {
          throw Exception(
            'فشل في إعادة تعيين كلمة السر (${response.statusCode})',
          );
        }
      }
    } catch (e) {
